...
graphRoute.post("/graph/files", async (c) => {
	try {
		...
		logger.error("Graph add file failed", {
...
graphRoute.delete("/graph/files/:cid", async (c) => {
	try {
		...
		logger.error("Graph remove file failed", {
			error: error instanceof Error ? error.message : String(error),
		});
		return c.json(
			{ error: error instanceof Error ? error.message : "Graph operation failed" },
...
graphRoute.post("/graph/connections", async (c) => {
	try {
		...
		logger.error("Graph connect files failed", {
			error: error instanceof Error ? error.message : String(error),
		});
		return c.json(
			{ error: error instanceof Error ? error.message : "Graph operation failed" },
...
graphRoute.delete("/graph/connections", async (c) => {
	try {
		...
		logger.error("Graph disconnect files failed", {
			error: error instanceof Error ? error.message : String(error),
		});
		return c.json(
			{ error: error instanceof Error ? error.message : "Graph operation failed" },
...
graphRoute.post("/graph/agents", async (c) => {
	try {
		...
		logger.error("Graph connect agent failed", {
			error: error instanceof Error ? error.message : String(error),
		});
		return c.json(
			{ error: error instanceof Error ? error.message : "Graph operation failed" },
...
graphRoute.get("/graph/traverse/:cid", async (c) => {
	try {
		...
		logger.error("Graph traverse failed", {
			error: error instanceof Error ? error.message : String(error),
		});
		return c.json(
			{ error: error instanceof Error ? error.message : "Graph operation failed" },
...
graphRoute.get("/graph/view", async (c) => {
	try {
		...
		const limit = parseInt(c.req.query("limit") || "3000", 10);

		const result = await getGraph({ walletAddress, limit: Math.min(limit, 3000) });
		return c.json(result);
	} catch (error) {
		logger.error("Graph view failed", {
			error: error instanceof Error ? error.message : String(error),
		});
		return c.json(
			{ error: error instanceof Error ? error.message : "Graph operation failed" },
